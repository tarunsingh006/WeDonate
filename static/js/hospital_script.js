...
// Fallback for different template structures
if (!appointmentCount) {
...
window.onload = function (){
    fetchCounts();
    // Also try to load appointments and donations immediately
    setTimeout(function() {
        fetchAppointments();
        fetchDonations();
    }, 1000);
...
var xhttp = new XMLHttpRequest();
        var getObject;
        const url = "/hospitals/fetch-counts/";
        xhttp.onreadystatechange = function() {
             if (this.readyState == 4 && this.status == 200) {
                 console.log('Raw response:', this.responseText);
                 getObject = JSON.parse(this.responseText);
                 console.log('Parsed response:', getObject);
                 
                 count_of_appointments = getObject[0].appointment_count;
                 count_of_donations = getObject[0].donation_count;
                 
                 console.log('Appointment count:', count_of_appointments);
                 console.log('Donation count:', count_of_donations);
                 
                 if (appointmentCount) {
                     appointmentCount.textContent = count_of_appointments;
                     console.log('Updated appointmentCount element');
                 } else {
                     console.log('appointmentCount element not found');
                 }
                 if (donationCount) {
                     donationCount.textContent = count_of_donations;
                     console.log('Updated donationCount element');
                 } else {
                     console.log('donationCount element not found');
                 }
                 
                 // Also update any other count displays
                 var appointmentCountAlt = document.getElementById('appointment-count');
                 var donationCountAlt = document.getElementById('donation-count');
                 if (appointmentCountAlt) {
                     appointmentCountAlt.textContent = count_of_appointments;
                     console.log('Updated appointment-count element');
                 }
                 if (donationCountAlt) {
                     donationCountAlt.textContent = count_of_donations;
                     console.log('Updated donation-count element');
                 }

             } else if (this.readyState == 4) {
                 console.error('Error fetching counts:', this.status, this.responseText);
             }
        };
...
var xhttp = new XMLHttpRequest();
        var getObject;
        const url = "/hospitals/fetch-appointments/";
        //var searchParam = "?" + "keyword=" +searchInput;
        //console.log(url+searchParam);
        xhttp.onreadystatechange = function() {
             if (this.readyState == 4 && this.status == 200) {
                 console.log('Appointments raw response:', this.responseText);
                 getObject = JSON.parse(this.responseText);
                 console.log("Number of appointments: ",getObject.length);
                 console.log("Appointments data:", getObject);
                 ...
                 console.error('Error fetching appointments:', this.status, this.responseText);
             }
        };
        xhttp.open("GET",url, true);
        xhttp.send();
...
function acceptAppointment(){
   ...
   if(!isRowSelectedForAppointment()){
      ...
      createPOSTRequest("/hospitals/appointments-approval/",true,true,appointmentID);
...
function denyAppointment(){
   ...
   if(!isRowSelectedForAppointment()){
     ...
     var radiosAppointment = document.getElementsByClassName("appointRadios");
      ...
      createPOSTRequest("/hospitals/appointments-approval/",true,false,appointmentID);
   }
}
...
function detailsOfAppointment(){
        ...
        var xhttp = new XMLHttpRequest();
        var getObject;
        const url = "/hospitals/fetch-appointment-details/";
...
var xhttp = new XMLHttpRequest();
var getObject;
        const url = "/hospitals/fetch-donations/";
        //var searchParam = "?" + "keyword=" +searchInput;
        console.log(url);
        xhttp.onreadystatechange = function() {
             if (this.readyState == 4 && this.status == 200) {
...
function acceptDonation(){
   if(!isDonationRowSelected()){
     ...
     //Fetching the donation ID of the selected row
      ...
      createPOSTRequest("/hospitals/donations-approval/",false,true,donationID);
...
function denyDonation(){
   ...
   if(!isDonationRowSelected()){
      ...
      createPOSTRequest("/hospitals/donations-approval/",false,false,donationID);
   }
}
...
function detailsOfDonation(){
        ...
        var xhttp = new XMLHttpRequest();
        var getObject;
        const url = "/hospitals/fetch-donation-details/";
...
function performSearch(){
    ...
    if(checkSearchInput(searchInput)){
        var xhttp = new XMLHttpRequest();
        var getObject;
        const url = "/hospitals/search-donations/";
        var searchParam = "?" + "keyword=" +searchInput;
        console.log(url+searchParam);
        xhttp.onreadystatechange = function() {
             if (this.readyState == 4 && this.status == 200) {
...
var xhttp = new XMLHttpRequest();
        ...
        const url = "/hospitals/search-donation-details/";
        var searchParam = "?" + "donation_id=" +donationId;
        console.log(url+searchParam);
        var getObject;
        xhttp.onreadystatechange = function() {
...
downloadPDF.onclick = function(){
     ...
     var URL_ = "/hospitals/view-pdf/" + donationId + "/";
...
function emailDonor(){
    if(isSearchRowSelected()){
     ...
     var URL_ = "/hospitals/email-donor/" + donationId + "/";
...
function getProfileDetails(){
  ...
  var xhttp = new XMLHttpRequest();
  var url = "/hospitals/get-user-details/";
  var hospital;
  xhttp.onreadystatechange = function() {
             if (this.readyState == 4 && this.status == 200) {
...
function updateProfileDetails(){
  var url = "/hospitals/update-user-details/";
...
function updatePasswordDetails(){
  ...
  var url = "/hospitals/update-pwd-details/";
